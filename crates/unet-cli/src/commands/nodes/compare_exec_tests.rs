...
mod tests {
    ...
    async fn test_compare_two_nodes_all() {
        ...
        let args = CompareNodeArgs { node_a: node.id, node_b: Some(Uuid::new_v4()), compare_type: vec![CompareType::All], diff_only: false };
    ...
    async fn test_compare_historical() {
        ...
        let args = CompareNodeArgs { node_a: Uuid::new_v4(), node_b: None, compare_type: vec![CompareType::Interfaces], diff_only: true };
        let result = compare_nodes(args, &store, crate::OutputFormat::Json).await;
        assert!(result.is_ok());
    }
}
