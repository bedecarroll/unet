...
impl PolicyEvaluationTask {
    ...
    async fn run(&self) {
        info!("Starting policy evaluation background task with interval: {}s", self.interval_seconds);
    ...
    async fn evaluate_all_policies(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let nodes = self.datastore.get_nodes_for_policy_evaluation().await
        ...
        // Load policies
        let policies = policy_service.load_policies().await
        ...
        for node in &nodes {
            match policy_service.evaluate_node(&*self.datastore, node).await {
                Ok(results) => {
                    ...
                    if let Err(e) = policy_service.store_results(&*self.datastore, &node.id, &results).await {
                        warn!("Failed to store policy results for node {}: {}", node.id, e);
                    }
                    
                    debug!("Evaluated {} policies for node {} ({})", 
                        results.len(), node.id, node.name);
                }
                Err(e) => {
                    failed_evaluations += 1;
                    error!("Failed to evaluate policies for node {} ({}): {}", 
                        node.id, node.name, e);
                }
            }
        }
        ...
        info!("Policy evaluation completed: {} nodes processed, {} successful, {} failed, {} total results, took {:?}",
            nodes.len(), successful_evaluations, failed_evaluations, total_results, duration);
...
impl SnmpPollingTask {
    ...
    pub async fn run(&self) {
        info!("Starting SNMP polling background task with interval: {}s", self.interval_seconds);
