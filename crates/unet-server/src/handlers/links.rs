...
use crate::api::{
    ApiResponse, CreateLinkRequest, UpdateLinkRequest, PaginatedResponse
};
...
pub async fn update_link(
    ...
    let interface_a = request.interface_a.unwrap_or_else(|| "updated-interface".to_string());
    
    let link = if let (Some(node_z_id), Some(interface_z)) = (request.node_z_id, request.interface_z) {
        ...
        // Internet circuit
        Link::new_internet_circuit(
            request.name.unwrap_or_else(|| "updated-internet-circuit".to_string()),
            node_a,
            interface_a,
        )
    };
